...
TaskNode::operator () () const {
		CirGate* p = cirMgr->getGate(_name);
		size_t val0 = p->_fanin0->gateId^(p->_invPhase0<<4);
		size_t val1 = p->_fanin1->gateId^(p->_invPhase1<<4);
		if(val0 >= val1) return val0^(val1<<11);
		else             return val1^(val0<<11);
	 }		
...
CirMgr::writeAag(ostream& outfile) const
{
	...
	for(size_t i=0; i<I; ++i){
		outfile << 2*(_piList[i]->gateId) << endl;
	}
	for(size_t i=0; i<O; ++i){
		CirGate* p = _poList[i];
		outfile << 2*(p->_fanin0->gateId)+(p->_invPhase0) << endl;
	}
	for(size_t i=0; i<a; ++i){
		CirGate* p = _aigList[i];
		outfile << 2*(p->gateId) << " " << 2*(p->_fanin0->gateId)+(p->_invPhase0) << " " << 2*(p->_fanin1->gateId)+(p->_invPhase1) << endl;
	}
	for(size_t i=0; i<I; ++i){
		CirGate* p = _piList[i];
		if(p->symbol!="") outfile << "i" << i << " " << p->symbol << endl; 
	}
	for(size_t i=0; i<O; ++i){
		CirGate* p = _poList[i];
		if(p->symbol!="") outfile << "o" << i << " " << p->symbol << endl; 
	}
	outfile << 'c' << endl;
...
{
	mm=-1, ii=-1, ll=-1, oo=-1, aa=-1;
	mm = g->gateId;
	_piwrite.clear();
	_aigwrite.clear();
	g->writeFanin();
	
	outfile << "aag " << mm << " " << ii << " " << ll << " " << oo << " " << aa << endl;
	for(size_t i=0; i<ii; ++i){
		outfile << 2*(_piwrite[i]->gateId) << endl;
	}
	outfile << 2*(g->gateId) << endl;

	for(size_t i=0; i<aa; ++i){
		CirGate* p = _aigwrite[i];
		outfile << 2*(p->gateId) << " " << 2*(p->_fanin0->gateId)+(p->_invPhase0) << " " << 2*(p->_fanin1->gateId)+(p->_invPhase1) << endl;
	}
	for(size_t i=0; i<ii; ++i){
		CirGate* p = _piwrite[i];
		if(p->symbol!="") outfile << "i" << i << " " << p->symbol << endl; 
	}
	if(g->symbol!="") outfile << "o" << 0 << " " << g->symbol << endl;
	outfile << 'c' << endl;
	outfile << "Write gate (" << g->gateId << ") by Chung-Yang (Ric) Huang" << endl;
