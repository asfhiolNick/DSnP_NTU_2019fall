...
DBAppendCmd::exec(const string& option)
{
   // TODO...
   // check option
   string now_key = "", now_val= "";
   int    now_int = 0;
   myStrGetTok(option, now_key);
   myStrGetTok(option, now_val);
   myStr2Int(now_val, now_int);
   DBJsonElem nowElem(now_key, now_int);
   dbjson.add(nowElem);
...
{  
   ...
   if (!CmdExec::lexSingleOption(option, token))
      return CMD_EXEC_ERROR;
   if(token=="") cout << dbjson;
   else          cout << "I haven't do the case: key is given "<< endl; 
   return CMD_EXEC_DONE;
}
