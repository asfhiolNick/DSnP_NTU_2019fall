...
{
   // TODO...
   if(_dofile->is_open()==true){
   	   _dofileStack.push(_dofile);
   } 
   else                        {}
   _dofile = new ifstream(dof.c_str());
   
   if(_dofile->is_open()==true) {return true;}
   else{
   	   _dofile->close();
	   return false; 
   }
...
CmdParser::parseCmd(string& option)
{
   ...
   size_t pointer=0;
   string command;
   pointer = myStrGetTok(str, command);
   CmdExec* e = getCmd(command);
   
   int size1 = str.size();
   int size2 = command.size();
   if(size1 > size2){
    option = str.substr(pointer, size1);
   }
   
   if(e !=0)    {return e;}
   else         {cerr << "Illegal behavior in the parseCmd!" << endl; assert(str[0]!=0 && str[0]!=' '); return NULL;}
...
CmdParser::getCmd(string cmd)
{
   ...
   string key = it -> first;
   string temp = key + ((it-> second)->getOptCmd()  );
   if( myStrNCmp(temp, cmd, key.size()) == 0 ) return it->second;
	} 
    return e;
}
