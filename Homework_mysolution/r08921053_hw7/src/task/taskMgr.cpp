...
TaskMgr::add(size_t nMachines)
{
   ...
   for(size_t i=0; i<nMachines; ++i){
   	   TaskNode* element = new TaskNode;
   	   while(_taskHash.insert(*element)==false){
   	   	    element = new TaskNode;
		  }
   	   _taskHeap.insert(*element);
   	   cout << "Task node inserted: " << *element << endl; 
   } 
}
...
{
   // TODO...
   TaskNode* element = new TaskNode(s, l);
   if(_taskHash.insert(*element) == true){
   	   _taskHeap.insert(*element);
   	   cout << "Task node inserted: " << *element << endl; 
   	   return true;
   }
...
{
   // TODO...
   if(taskMgr->empty()==true) return false;
   TaskNode top = _taskHeap.min();
   top += l;
   _taskHash.update(top);
   
   _taskHeap.delMin();
   _taskHeap.insert(top); 
   return true;
}
